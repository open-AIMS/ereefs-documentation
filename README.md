...
MongoDB to status COMPLETE once all tasks are finished (See the bottom of the chart). Also, in the [Job subsection of Status Descriptions](#job), it says that job status COMPLETE "can only be set by JobTerminator", however this chart shows that TaskHandlerController run `updateJobStatus(compelete)`. Maybe the message-flow diagram should be 
